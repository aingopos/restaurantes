...
import entidades.DetalleFacturaFacade;
...
public class DetalleFacturaController implements Serializable {
    ...
    @EJB
    private entidades.DetalleFacturaFacade ejbFacade;
    ...
    protected void initializeEmbeddableKey() {
        selected.setDetalleFacturaPK(new entidades.DetalleFacturaPK());
    }
    ...
    public static class DetalleFacturaControllerConverter implements Converter {
        ...
        entidades.DetalleFacturaPK getKey(String value) {
            entidades.DetalleFacturaPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new entidades.DetalleFacturaPK();
            key.setPedCodigo(Integer.parseInt(values[0]));
            key.setFacCodigo(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(entidades.DetalleFacturaPK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getPedCodigo());
            sb.append(SEPARATOR);
            sb.append(value.getFacCodigo());
            return sb.toString();
        }
